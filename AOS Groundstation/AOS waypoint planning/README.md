...
- **public/**  
  Contains `index.html` plus any CSS, JS, or image assets.  
- **AOS Server**  
  Contains `AOS_Broker.exe`, which will be launched by `launcher.mjs`.
...
1. Open a terminal (Command Prompt or PowerShell on Windows) and change into the folder containing `launcher.js`:
   ```bash
...
3. Open `package.json` in a text editor and add `"type": "module"` at the top level. Example:
   ```jsonc
    {
...
From the same directory (`AOS waypoint planning`), run:
...
- It will also create `node_modules/` and `package-lock.json`.
...
#### Note: The rest is done automatically by bat file (**.../AOS Groundstation/main.bat**). Incase if you want to try out separately, you can follow the following instructions. 
