...
Prerequisites
Node.js
Download the latest LTS release (v14.x or newer) from https://nodejs.org and install it.
...
Go to https://nodejs.org and download the latest LTS installer.
Follow the installer prompts.
Confirm installation:
node --version   # e.g. v18.16.0
npm --version    # e.g. 9.5.0
2. Initialize npm & Enable ES Modules
Open a terminal (Command Prompt or PowerShell on Windows) and change into the folder containing launcher.mjs file, public and API folders:
cd "<FILE DIRECTORY>\AOS Groundstation\AOS waypoint planning"
Run:
npm init -y
This creates a barebones package.json.
Open package.json in a text editor and add make sure "main": "launcher.mjs" is there, Example:
 {
...
http://localhost:3000/
You should see the waypoint planning module.
...
Autonomous Check
Set the Collision Threshold (minimum separation distance).
Click Detect Collisions.
The tool will compute any collision points based on each waypoint’s location, speed, and holding time, and highlight them on the map.
1750658529438
...
After the initial takeoff, any subsequent mission uploads will be applied automatically when it is clicked Send Missionbutton without additional manual steps.
6. Takeoff
