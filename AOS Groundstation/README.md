...
## How to use it
After installing all modules on the PC and the DJI app on the remote controller(s), and after making sure that all devices are in the same network, you need to start AOS_Groundstation.bat inside AOS Server folder as administrator. It will automatically launch all modules on the PC: server, map visualization, and waypoint mission planning. Then you need to connect your drones to the server. After all drones are connected, you can run AOS Broker.py inside AOS Server folder. After running python code, you need to plan your waypoint mission and transmit it to the server (see AOS Waypoint Mission Planning). You will see the mission parameters on the terminal where you run the python code. Then you need to press **T** for the drones to take off and execute the first mission (After pressing **T** they will increase their altitude vertically from the position you took off. The altitude is the altitude of the first waypoint of the mission). If you want to continue with the new mission after all drones complete the first mission, you need to design a new mission and send it to the server. After sending the new mission, drone(s) will execute it right away. Later when you decide to land, you need to press **L** for the drones to return to their initial altitudes and positions above the initial take-off position and land them. Note, that for take-off and landing, the first and last meters have to be flown manually for safety reasons. So the drones will not directly take off from or land to the ground. The provided python code in  AOS Server gives an example on how to use all of these modules in own projects.   
