/*class Camera {
    ...
    async capture2(preview) {
        const index = this.captures.length;
        const image = new Image2(this, index);
        return await image.capture(preview);
    }
    async capture3(preview) {
        const index = this.captures.length;
        const image = new Image3(this, index);
        return await image.capture(preview);
    }
    async capture4(preview) {
        const index = this.captures.length;
        const image = new Image4(this, index);
        return await image.capture(preview);
    }
    async capture5(preview) {
        const index = this.captures.length;
        const image = new Image5(this, index);
        return await image.capture(preview);
    }
    async capture6(preview) {
        const index = this.captures.length;
        const image = new Image6(this, index);
        return await image.capture(preview);
    }
    async capture7(preview) {
        const index = this.captures.length;
        const image = new Image7(this, index);
        return await image.capture(preview);
    }
    async capture8(preview) {
        const index = this.captures.length;
        const image = new Image8(this, index);
        return await image.capture(preview);
    }
    async capture9(preview) {
        const index = this.captures.length;
        const image = new Image9(this, index);
        return await image.capture(preview);
    }
    ...
    async export1(zip) {
        ...
        this.captures.forEach((capture, i) => {
            data.captures.push({
                imageC1: capture.image,
                center: capture.center
            });

            // export images
            const base64 = canvasImage(capture.canvas.full);
            const filename = `imageC1-${capture.image}-${this.config.drone.camera.type}.png`;
            camera.file(filename, base64, { base64: true });
    ...
    async export2(zip) {
        ...
        this.captures.forEach((capture, i) => {
            data.captures.push({
                imageC2: capture.image,
                center: capture.center
            });

            // export images
            const base64 = canvasImage(capture.canvas.full);
            const filename = `imageC2-${capture.image}-${this.config.drone.camera.type}.png`;
            camera.file(filename, base64, { base64: true });
    ...
    async export3(zip) {
        ...
        this.captures.forEach((capture, i) => {
            data.captures.push({
                imageC3: capture.image,
                center: capture.center
            });

            // export images
            const base64 = canvasImage(capture.canvas.full);
            const filename = `imageC3-${capture.image}-${this.config.drone.camera.type}.png`;
            camera.file(filename, base64, { base64: true });
    ...
    async export4(zip) {
        ...
        this.captures.forEach((capture, i) => {
            data.captures.push({
                imageC4: capture.image,
                center: capture.center
            });

            // export images
            const base64 = canvasImage(capture.canvas.full);
            const filename = `imageC4-${capture.image}-${this.config.drone.camera.type}.png`;
            camera.file(filename, base64, { base64: true });
    ...
    async export5(zip) {
        ...
        this.captures.forEach((capture, i) => {
            data.captures.push({
                imageC5: capture.image,
                center: capture.center
            });

            // export images
            const base64 = canvasImage(capture.canvas.full);
            const filename = `imageC5-${capture.image}-${this.config.drone.camera.type}.png`;
            camera.file(filename, base64, { base64: true });
    ...
    async export6(zip) {
        ...
        this.captures.forEach((capture, i) => {
            data.captures.push({
                imageC6: capture.image,
                center: capture.center
            });

            // export images
            const base64 = canvasImage(capture.canvas.full);
            const filename = `imageC6-${capture.image}-${this.config.drone.camera.type}.png`;
            camera.file(filename, base64, { base64: true });
    ...
    async export7(zip) {
        ...
        this.captures.forEach((capture, i) => {
            data.captures.push({
                imageC7: capture.image,
                center: capture.center
            });

            // export images
            const base64 = canvasImage(capture.canvas.full);
            const filename = `imageC7-${capture.image}-${this.config.drone.camera.type}.png`;
            camera.file(filename, base64, { base64: true });
    ...
    async export8(zip) {
        ...
        this.captures.forEach((capture, i) => {
            data.captures.push({
                imageC8: capture.image,
                center: capture.center
            });

            // export images
            const base64 = canvasImage(capture.canvas.full);
            const filename = `imageC8-${capture.image}-${this.config.drone.camera.type}.png`;
            camera.file(filename, base64, { base64: true });
    ...
    async export9(zip) {
        ...
        this.captures.forEach((capture, i) => {
            data.captures.push({
                imageC9: capture.image,
                center: capture.center
            });

            // export images
            const base64 = canvasImage(capture.canvas.full);
            const filename = `imageC9-${capture.image}-${this.config.drone.camera.type}.png`;
            camera.file(filename, base64, { base64: true });
