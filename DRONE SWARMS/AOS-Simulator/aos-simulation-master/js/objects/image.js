...
class Image1 {
    ...
    async capturePersons(integrate) {
        ...
        // get persons centers
        const centers = positions.map((position) => {
            const rendered = new THREE.Vector3(Math.fround(position.x, 0, position.z));
            const processed = this.translate(rendered);
            return {
                rendered: rendered,
                processed: processed
    ...
    async captureImage(integrate) {
        const rendered = new THREE.Vector3(Math.fround(this.center.x, 0, this.center.z));
        const processed = this.translate(rendered);
